...
metadata round trips that the system already had the answers to cuts a
150-file read by 47\%.
\end{abstract}
...
Much of the tabular data an organisation analyses does not live in an
...
\texttt{gdrive://} scheme, so \texttt{read\_parquet}, \texttt{read\_csv},
\texttt{glob}, \texttt{COPY}, and the DuckLake lakehouse format all inherit it
without change. The design work is almost entirely in the caches and retry
rules; Section~\ref{sec:design} describes both, attaching each choice to the
measurement that produced it, including the ones we got wrong first.
...
\item A set of negative results --- a read-ahead buffer that an external review
  recommended and that measurement refuted, and a block size that was noise ---
  reported because each was the natural thing to believe.
\end{itemize}
...
opens one handle per scan thread. On a 32-thread machine we measured 19
...
an id across a delete-and-recreate. Serving that refresh from a time-based
cache therefore lets a file deleted and recreated between two queries be read
...
\emph{after} the handle was opened. It is not strictly better, though ---
...
omits Drive. The most likely first experience of the default credential
provider is therefore an error that, without the split, sends the user to
audit Drive sharing for a problem entirely inside their token. Storage-quota
errors are likewise kept distinct from rate limits:
...
network: AMD Ryzen 9 3950X, 16 cores / 32 threads, 125\,GiB RAM, Linux 6.18,
on a 485\,Mbit/s domestic link in northern Germany. DuckDB v1.5.5, extension
v2026.08.01. The link matters: it is fast enough that none of the results
below are bandwidth-bound --- the 87\,MB fixture would cross it in 1.4\,s at
line rate --- but it is not a datacentre, so absolute latencies would be lower
from a cloud VM. The comparisons are all same-machine, same-session, so the
link cancels. The corpus for the single-file experiments is one 87\,MB
...
quarters of the write is directory scaffolding.} The cost is also one-off per
data path rather than per row, so it would be invisible in a throughput
number. The recommendation it produces --- create the directory tree once,
then write into it --- is accordingly not the one a bandwidth measurement
would suggest.
...
backend, three columns aggregated. Nine repeats, \textbf{all cloud legs in one
session}. Ratios are against the GCS minimum. Download-then-query was measured
separately, three repeats on the same machine within the hour.}
\label{tab:coldscan}
\small
\begin{tabular}{lrrr}
\toprule
Backend & min & median & vs.\ GCS \\
\midrule
Local disk                          & 0.11\,s & 0.12\,s & 0.08$\times$ \\
\texttt{s3://} (eu-central-1)       & 0.97\,s & 1.42\,s & 0.72$\times$ \\
\texttt{gs://} (europe-west3)       & 1.35\,s & 1.55\,s & 1.00$\times$ \\
\texttt{gdrive://}, defaults        & 3.93\,s & 5.34\,s & \textbf{2.91$\times$} \\
\texttt{gdrive://}, 128\,MiB blocks & 2.60\,s & 5.28\,s & \textbf{1.93$\times$} \\
\addlinespace
Download, then query locally        & 6.14\,s & 9.70\,s & 4.55$\times$ \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:coldscan} gives the headline result: roughly 3$\times$ object
storage at the defaults, 2$\times$ tuned, and 36$\times$ a local file. The
workflow this replaces --- fetch the bytes, then scan them --- is slower than
querying in place, which is the practical case for the extension and the only
row here that is not a cost.

\textbf{Do not read the 2.91$\times$ as an improvement.} We set a 3$\times$ gate
before measuring, and this run passes it where an earlier one recorded
3.05$\times$. Nothing in the extension got faster between them: the benchmark
addresses its fixture through the \texttt{gdrive://id:} form, which skips path
resolution entirely, so none of Section~\ref{sec:evalrefresh}'s work touches
this number. The denominator moved. GCS answered in 1.35\,s here and 1.34\,s
there, which is not the difference; the Drive leg drifted from 4.09\,s to
3.93\,s, well inside its own run-to-run spread. The verdict changed, the system
did not.

\textbf{The denominator is the least stable number in the table.} S3 in
Frankfurt answers the same request in 0.97\,s against GCS's 1.35\,s --- a 39\%
gap between two object stores in the same city, measured minutes apart. Quoted
against S3 the extension is 4.05$\times$ rather than 2.91$\times$, and the gate
would fail. We report GCS because that is the denominator the gate was written
against, not because it is the flattering one; had we chosen after seeing the
data, the number would deserve no trust at all. The medians tell the same
story less kindly: at 5.34\,s the Drive leg's median is 3.4$\times$ the GCS
median, because Drive's spread is wide and one slow request moves a whole run.
...
local in all three. Minimum of three runs, each operation in a fresh process
against a persistent catalog. 200\,000 rows, then a 50\,000-row append.}
\label{tab:ducklake}
\small
\begin{tabular}{lrrrr}
\toprule
Operation & \texttt{s3://} & \texttt{gs://} & \texttt{gdrive://} & vs.\ GCS \\
\midrule
\texttt{CREATE TABLE AS} & 1.51\,s & 1.55\,s & 7.80\,s & 5.0$\times$ \\
\texttt{INSERT} (append) & 0.59\,s & 0.66\,s & 3.54\,s & 5.4$\times$ \\
\texttt{DELETE}          & 0.68\,s & 1.03\,s & 6.39\,s & 6.2$\times$ \\
Scan, cold               & 0.61\,s & 0.92\,s & 3.70\,s & 4.0$\times$ \\
Time-travel read         & 0.24\,s & 0.25\,s & 2.61\,s & \textbf{10.4$\times$} \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:ducklake} is the experiment the rest of the paper exists to
support, and it is less flattering than Table~\ref{tab:coldscan}. A single
large scan runs at 2.9$\times$ GCS; a lakehouse workload runs at 4--10$\times$.

The spread across operations is the finding. The cheapest operation on an
object store is the most expensive here in relative terms: a time-travel read
costs 0.25\,s on GCS and 2.61\,s on Drive, \textbf{10.4$\times$}. That query
reads almost no data --- it resolves a snapshot and reads one small file --- so
it is nearly pure metadata, and metadata is exactly where Drive is weakest.
The full scan, which moves the most bytes, has the \emph{best} ratio at
4.0$\times$. Drive's penalty is not proportional to data volume; it is
proportional to how many things you have to look up, which is the same
conclusion Figure~\ref{fig:flame} reached from a different direction.

\textbf{Two object stores, one city, 39\% apart.} S3 and GCS are both in
Frankfurt and differ by more than a third on several rows. That is worth
stating because it bounds how precisely any of these ratios can be read: the
gap between the two \emph{baselines} is a substantial fraction of the effects
being measured on them.

\subsection{DuckLake's Caching Model}

DuckLake, at the version measured here, \textbf{exposes no local data-file
cache of its own}. Enumerating \texttt{duckdb\_settings()} against
\texttt{ducklake} \texttt{d8a1881e} returns retry and inlining knobs and
nothing about caching data files. The relevant control is DuckDB core's
\texttt{enable\_external\_file\_cache}, an \emph{in-memory} cache of external
file data, on by default; the separate \texttt{duckdb-diskcache} extension is
a different mechanism that we did not measure.

We expected this to matter disproportionately on Drive, because a cache is
worth more when a miss costs 1.2\,s than when it costs milliseconds. It does
not matter at all:

\begin{table}[t]
\caption{Cold scan with \texttt{enable\_external\_file\_cache} on and off,
minimum of three runs.}
\label{tab:dlcache}
\small
\begin{tabular}{lrrr}
\toprule
Backend & cache on & cache off & difference \\
\midrule
\texttt{s3://}     & 0.61\,s & 0.59\,s & $-$0.02\,s \\
\texttt{gs://}     & 0.92\,s & 0.91\,s & $-$0.01\,s \\
\texttt{gdrive://} & 3.70\,s & 3.62\,s & $-$0.08\,s \\
\bottomrule
\end{tabular}
\end{table}

Every difference is negative and smaller than the run-to-run spread --- the
cache is not merely unhelpful here, it is invisible. The reason is structural
rather than a property of Drive: the cache lives in memory, and every
measurement starts a fresh process. A cold query cannot hit an in-memory cache
that no longer exists. Our hypothesis was wrong for a reason we should have
seen before measuring, and it holds equally for all three backends.

This matters for advice. On Drive the expensive thing is the \emph{first}
access from a new process, and an in-memory cache never helps that. Only a
cache that survives the process --- on disk, or a long-lived server holding the
connection and the resolved ids --- would move these numbers. That is the shape
of the remaining headroom, and it is not something the storage layer can supply
on its own.
...
\begin{table}[t]
\caption{Write path, minimum of three cold runs. Sizes are the uncompressed
row payload; Parquet compresses, so the effective rate is what the user waits
for, not a disk figure.}
\label{tab:writes}
\small
\begin{tabular}{lrr}
\toprule
...
\midrule
Upload 1\,MiB                & 3.99\,s  & 5.42\,s \\
Upload 16\,MiB               & 10.01\,s & 12.18\,s \\
Upload 128\,MiB              & 63.95\,s & 66.39\,s \\
Create one folder + tiny file & 3.69\,s  & 5.08\,s \\
\bottomrule
\end{tabular}
\end{table}

Writes behave unlike reads. A ranged read costs the same whatever its length;
an upload does not. Between 1\,MiB and 128\,MiB the time grows roughly with
size --- 16$\times$ the bytes for 16$\times$ the time from 16 to 128\,MiB ---
so uploads are bandwidth-bound where reads are latency-bound. The fixed cost
is still large: 3.99\,s to move one mebibyte, and 3.69\,s to create a folder
and a file of a few dozen bytes. Below roughly 16\,MiB an upload is mostly
overhead.

That asymmetry has a practical consequence for a lakehouse on Drive, and it
runs opposite to the usual advice. On an object store, many small files are
the pathology. Here the per-request floor makes small writes disproportionately
expensive while large ones stream at a reasonable rate, so a writer should
prefer fewer, larger data files --- which is also what makes the reads cheap
(Section~\ref{sec:evalrefresh}).

\begin{figure}[t]
\centering
\input{fig_threads}
\caption{Thread scaling, minimum of three cold runs. Wall-clock (solid, left
axis) falls from 9.10\,s to 4.18\,s; Drive API calls (dashed, right axis) stay
at eight. The flat line is the assertion: decompression parallelises, and the
single-flight caches keep the request count independent of thread count. Before
those caches existed the same sweep produced 3, 6 and 19 metadata calls at 1, 4
and 32 threads.}
\label{fig:threads}
\end{figure}
...
that fails for Drive as a Workspace API (Section~\ref{sec:auth}); the latter
addresses a single native format rather than a filesystem. Against
...
format on Drive is not a workaround but the natural fit. The same immutability
that makes it correct is directly exploitable for speed: removing 150 metadata
round trips, with the glob-cache fix alongside it, cuts 47\% of wall-clock
from a 150-file read. The
