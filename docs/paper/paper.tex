...
stale, and each adds a component that has to be provisioned and watched.
...
which makes the fine-grained ranged access that Parquet is designed around the
worst possible request pattern. Section~\ref{sec:background} measures all
three.
...
caches and in the rules about what may be retried;
Section~\ref{sec:design} describes both, and every choice is attached to the
...
therefore not a performance question but a correctness question, and it is not
answered by observing that the tests pass. Section~\ref{sec:guarantees} sets
out the two contracts side by side and attaches a verification experiment to
every row, including the rows where the honest answer is that Drive's
documentation does not say and we have not yet measured it.
...
decision in Section~\ref{sec:design} follows from one of them.
...
defined entirely in terms of paths. Bridging that gap cheaply is the single
largest source of avoidable work in the system, and it is the risk we
identified as most likely to sink the project before we wrote any code.
...
that does not exist --- but ``a ranged read of a large object'', and that is
precisely the request a Parquet scan is made of. A total, though, does not say
whether the cost is addressable. Section~\ref{sec:breakdown} therefore
decomposes each request kind into connection setup, server time and transfer,
and places the resulting spans on a wall-clock timeline; that decomposition,
not the totals here, is what tells us which part of the floor is ours to
remove and which belongs to Google.
...
support \texttt{Range}. Any filesystem over Drive must therefore decide what a
...
\texttt{glob}, \texttt{COPY}, and, importantly for this paper, DuckLake's
...
One consequence of the per-instance registration is worth stating early
because it constrains everything below: a single filesystem object serves
every \texttt{ClientContext} in the process, so every cache it owns is shared
across databases, users and credentials. Cache keys must therefore carry
identity, not merely a path.
...
is cheap --- which is in turn why this cache is freely evictable while the
block cache of Section~\ref{sec:blockcache} is capped separately.
...
write guard would then have created a third duplicate.
...
32 $\rightarrow$ 19) while the number of data requests stayed at 35. Caching
metadata is obviously right; the interesting part is the scope.
...
query cannot see the previous query's metadata, and a query executing with no
context caches nothing at all. Absent files are never cached, so a later
create is immediately visible.

\textbf{Refreshing metadata that cannot have changed.}
\label{sec:skiprefresh}
Opening a file needs its current size and \texttt{headRevisionId}, and the
obvious implementation asks Drive for them --- one \texttt{files.get}, 268\,ms
(Figure~\ref{fig:breakdown}), per file per query. The timeline in
Figure~\ref{fig:flame} is what showed this was often pure waste. DuckDB's
multi-file reader \emph{globs} a path before opening it, and a glob is
\texttt{files.list} calls whose field mask already returns size and
\texttt{headRevisionId}. The refresh therefore re-asks, over a second round
trip, for values the previous request already delivered.
...
stale. This is unconditional and changes no semantics: within one query, a
listed entry and a fetched one are the same bytes from the same source.
...
This one is a genuine trade and is off by default. The promise it asks for is
stronger than it first appears, and an independent review of this change is
what established how much stronger: not merely ``never overwritten in place''
but never modified, replaced, \emph{or} deleted-and-recreated at that path
while the process runs. Get it wrong and reads go stale with nothing to detect
it: Drive keeps a file's id across an overwrite, so there is
no \texttt{404}, and it offers no working validator either. We checked, because
a cheap validator would have made the setting unnecessary: Drive returns no
\texttt{ETag} on metadata or media responses, and an \texttt{If-Match} carrying
a deliberately wrong value is \emph{silently ignored} --- \texttt{206} with the
data, not \texttt{412}. Prefixes therefore match on whole path segments, never
characters, so declaring \texttt{lake} cannot quietly exempt
\texttt{lakehouse}.
...
by asking a reviewer that had not written the code, not by testing.
...
source claiming it was ``strictly better'' had been wrong since before this
work began.
...
\textbf{Two normalisations that are easy to get wrong.}
...
\textbf{The obvious alternative, measured and rejected.}
...
so the reuse was worth having for reasons we had not correctly identified. The
pool is per-thread rather than shared
because the underlying HTTP client owns its socket and DuckDB already hands
each thread its own file handle. The bearer token is a per-request header and
never a property of the connection, so reuse across secrets carries no
credential between identities.
...
into success by re-fetching the metadata. This is the one place where we can
recover the guarantee the transport does not give us, and it is worth the
extra round trip.
...
So the client also emits one span per API attempt --- start, end, thread,
call kind, HTTP status, response size, whether the attempt paid for a new
...
\textbf{It must cost nothing when off.} The enable flag is a function-local
...
obvious thing and is unsafe: two databases in one process, each with a secret
named \texttt{gdrive}, would share whichever token was minted first, while each
applied its own \texttt{drive\_id}. We key on
...
from SQL and therefore untested, which we would rather state than discover
later.
...
detect the dead id on the failing read and re-resolve once. Handling it at read
time is strictly better: it also covers a file deleted \emph{after} the handle
was opened, which open-time validation cannot see by construction.
...
Line those requirements up against the table. The rows Drive fails --- atomic
...
supplies, one of them (idempotent create) only because we reserve an id to get
it. The remaining mismatch is duplicate names, and DuckLake's UUID-based file
naming makes a collision vanishingly unlikely rather than merely rare.
...
turns the guarantee into a measurable saving rather than merely an absence of
harm. We regard this as the practical payoff of the thesis: the same property
that makes a lakehouse \emph{correct} on Drive is the one that makes it
\emph{fast} on Drive, and neither follows from the other by accident.
...
the previous run. Warm numbers would be flattering and would not describe what
a user experiences, which is the first scan.
...
the closest available estimate of the true cost. Nine rather than three is not
fussiness. The GCS denominator varies by 67\% between sessions --- we observed
1.03\,s to 1.72\,s for an identical measurement --- which is enough to move a
ratio by more than a full multiple and to flip a verdict on its own. Across
...
\textbf{What it means for optimisation.} The residual is Google's service
time, not transfer, and that closes off an entire class of client-side work.
...
what each costs. That is exactly the question that killed the read-ahead
buffer of Section~\ref{sec:blockcache}, where a per-handle optimisation was
invisible to a workload whose adjacency lay across handles, and it is the
question Table~\ref{tab:latency} cannot answer in principle.
...
before any of it is requested. This is Drive's missing path addressing (R-1)
appearing not as a request count but as a critical path, and it is the
strongest available argument for the \texttt{gdrive://id:} form and for keeping
...
rather than serialised --- a reminder that Figure~\ref{fig:breakdown}'s
per-request costs cannot be multiplied by a count to predict a total, which is
the mistake that produced this paper's original round-trip estimate. The call
count matters in its own right regardless: Drive enforces per-user API quotas
that object stores do not, so a query that costs 150 fewer requests is not
merely faster but cheaper against a limit users actually hit.
...
and it is why we report two labelled sessions rather than one merged table with
a single ratio. A paper that quoted ``2$\times$ object storage'' by pairing the
best Drive run with the slowest denominator would not be wrong about any
individual measurement, and would still be misleading.
...
Three findings, none of them obvious in advance. First, \textbf{fewer requests
help far less than the per-request floor predicts}. Collapsing 35 requests to
...
that is per-request latency or a request pattern we could improve.
